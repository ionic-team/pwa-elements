...
export class CameraPWA {
  ...
  async capture() {
    if (this.hasImageCapture()) {
      try {
        ...
        const finalPhoto = await this.renderFinalImage(photo);

        this.promptAccept(finalPhoto);
      } catch (e) {
  ...
  async promptAccept(photo: any) {
    this.photo = photo;
    console.log('Prompt accept of photo', photo);
    // this.photoSrc = URL.createObjectURL(photo);
    // console.log("Photo src", this.photoSrc);
  }

  renderFinalImage(photo: Blob): Promise<string> {
    console.log('Rendering blob', photo);
    return new Promise((resolve) => {
      const ctx = this.offscreenCanvas.getContext("2d");
      //ctx.translate(this.offscreenCanvas.offsetWidth, 0);
      //ctx.scale(-1, 1);

      const i = new Image();
      i.onload = () => {
        console.log('Drawing image on it', i.src, i);
        console.dir(i);
        ctx.drawImage(i, 0, 0);
        resolve(this.offscreenCanvas.toDataURL('image/jpeg'));
      }
  ...
  render() {
    ...
    return (
      <div class="camera-wrapper">
        ...
        {this.photo && (
        <div class="accept">
          <div class="accept-image" style={{...videoStreamStyle, backgroundImage: `url(${this.photoSrc})`}}></div>
        </div>
        )}
